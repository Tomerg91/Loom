/**
 * Prisma seed script for the Action Items & Homework domain.
 *
 * The TypeScript version exists for developer ergonomics while the runtime
 * script (`prisma/seed.js`) is executed by `npm run db:seed`. Keep the two files
 * in sync whenever updates are made.
 */
...
type CategoryKey = 'habitFoundations' | 'wellnessRecovery' | 'accountability';
...
type InstanceSeed = {
  ...
  progressUpdates?: ProgressUpdateSeed[];
  attachments?: AttachmentSeed[];
  notificationJobs?: NotificationJobSeed[];
};
...
type TaskSeed = {
  ...
  recurrenceRule?: Prisma.JsonObject;
...
/**
 * Anchor date (today at 17:00 UTC) used to generate deterministic offsets so
 * the sample data remains relative to the current day while staying idempotent
 * between successive seed executions.
 */
const BASE_DATE = (() => {
  const now = new Date();
  return new Date(
...
/**
 * Calculates a future or past date relative to {@link BASE_DATE} using the
 * provided day offset.
 */
...
/**
 * Computes a timestamp offset by a number of hours from the supplied base
 * date. Negative offsets move backwards (e.g., for overdue notifications).
 */
function dateFromHoursOffset(base: Date, hours: number): Date {
  return new Date(base.getTime() - hours * 60 * 60 * 1000);
}

/**
 * Ensures the demo task categories exist for the given coach, returning their
 * identifiers for downstream task creation.
 */
async function seedCategories(coachId: string) {
  const results: Record<CategoryKey, { id: string }> = {
    habitFoundations: { id: '' },
    wellnessRecovery: { id: '' },
    accountability: { id: '' },
  };
...
/**
 * Upserts demo tasks, recreating their instances, progress updates, and
 * notification jobs so the dataset showcases multiple workflow states.
 */
async function seedTasks(
  coachId: string,
  clientId: string,
  categories: Record<CategoryKey, { id: string }>
) {
...
/**
 * Seeds a representative export log entry so reporting screens have data to
 * render during development.
 */
async function seedExportLog(coachId: string, clientId: string) {
