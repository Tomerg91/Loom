...
try {
  ...
  console.error(
    `   ❌ OpenAPI TypeScript integration validation failed: ${error.message}`
  );
  process.exit(1);
}
...
try {
  ...
  if (!docPageContent.includes('/api/docs')) {
    throw new Error(
      'API documentation page not configured to fetch from /api/docs'
    );
  }
  ...
  console.error(
    `   ❌ API documentation page validation failed: ${error.message}`
  );
  process.exit(1);
}
...
try {
  ...
  console.error(
    `   ❌ Human-readable documentation validation failed: ${error.message}`
  );
  process.exit(1);
}
...
console.log(
  '   2. Visit the API documentation: http://localhost:3000/en/api-docs'
);
