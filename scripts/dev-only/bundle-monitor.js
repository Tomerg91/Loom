...
function displayResults(currentSize, comparison) {
  ...
  if (comparison.previous !== undefined) {
    ...
    log(
      `Previous: ${comparison.previous} KB → Current: ${comparison.current} KB`,
      color
    );
  }
...
function main() {
  ...
  try {
    const currentSize = isQuickMode
      ? getQuickBundleEstimate()
      : analyzeBundleSize();
    ...
    if (
      ...
      (process.argv.includes('--strict') || process.env.CI)
    ) {
