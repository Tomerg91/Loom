...
function analyzeBundleStats() {
  ...
  try {
    ...
    if (!fs.existsSync(analyzeDir)) {
      log(
        '❌ Bundle analysis files not found. Make sure ANALYZE=true is set during build.',
        'red'
      );
      return null;
    }
    ...
    return {
      totalSize: clientReport.assets.reduce(
...
function checkPerformanceTargets(stats) {
  ...
  if (stats.routeStats) {
    ...
    const totalSize = stats.routeStats.reduce(
      (total, route) => total + route.size,
      0
    );
    ...
    log(
      `   Total Size: ${formatBytes(totalSize)} / ${formatBytes(PERFORMANCE_TARGETS.totalBundleSize)}`
    );
    log(
      `   Target Met: ${bundleTargetMet ? '✅ YES' : '❌ NO'} (${formatPercentage(totalSize, PERFORMANCE_TARGETS.totalBundleSize)})`
    );

    if (bundleTargetMet) results.passed++;
    else results.failed++;
    ...
    for (const route of stats.routeStats) {
      ...
      log(
        `   ${indicator} ${route.route}: ${formatBytes(route.firstLoadJS)} / ${formatBytes(PERFORMANCE_TARGETS.firstLoadJS)}`
      );
    ...
    const overallFirstLoadPassed =
      firstLoadTargetsMet === firstLoadTargetsTotal;
    log(
      `\n   Overall First Load JS: ${overallFirstLoadPassed ? '✅ PASSED' : '❌ FAILED'} (${firstLoadTargetsMet}/${firstLoadTargetsTotal} routes)`
    );

    if (overallFirstLoadPassed) results.passed++;
    else results.failed++;
    results.details.push({
...
function generateOptimizationSuggestions(stats) {
  ...
  if (stats.routeStats) {
    // Find heavy routes
    const heavyRoutes = stats.routeStats.filter(
      route => route.firstLoadJS > PERFORMANCE_TARGETS.firstLoadJS
    );
    ...
    if (heavyRoutes.length > 0) {
      ...
      heavyRoutes.forEach(route => {
        ...
        suggestions.push(
          `   • ${route.route}: ${formatBytes(excess)} over target`
        );
      });
      suggestions.push(
    ...
    if (totalSize > PERFORMANCE_TARGETS.totalBundleSize) {
      ...
      suggestions.push(
        '   → Consider lazy loading for admin/analytics components'
      );
    }
    ...
    suggestions.push(
      '   • Enable dynamic imports for chart libraries (recharts)'
    );
  ...
  if (suggestions.length === 0) {
    ...
    log(
      '   Your bundle is well-optimized and meets performance targets.',
      'green'
    );
  } else {
...
function main() {
  ...
  log(
    `\n${statusIcon} Overall Status: ${overallPassed ? 'PASSED' : 'FAILED'}`,
    statusColor
  );
  log(
    `   Targets Met: ${report.summary.targetsMet}/${report.summary.targetsTotal}`,
    statusColor
  );
  ...
  if (!overallPassed) {
    ...
    log(
      '   Review the suggestions above and implement optimizations.',
      'yellow'
    );
  } else {
