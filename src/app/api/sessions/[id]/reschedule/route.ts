...
export const POST = withErrorHandling(
  async (
    ...
    if (!session) {
      return propagateCookies(
        authResponse,
        createErrorResponse('Session not found', HTTP_STATUS.NOT_FOUND)
      );
    }
    ...
    if (user.id !== session.coachId && user.id !== session.clientId) {
      return propagateCookies(
        authResponse,
        createErrorResponse('Forbidden', HTTP_STATUS.FORBIDDEN)
      );
    }
    ...
    try {
      ...
      const otherPartyId =
        user.id === session.coachId ? session.clientId : session.coachId;
    ...
    return propagateCookies(
      authResponse,
      createSuccessResponse(updatedSession, 'Session rescheduled successfully')
    );
  }
);
