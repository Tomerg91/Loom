...
-- Execute this script in Supabase Dashboard > SQL Editor to fix recursive policies
...
-- FROM pg_policies 
-- WHERE definition LIKE '%users%' AND definition LIKE '%auth.uid()%';
...
-- 1. Test that you can query the users table without infinite recursion
-- 2. Verify that admin users can still access all data appropriately
-- 3. Ensure that the JWT contains the 'role' field for users
-- 4. If role is not in JWT, you'll need to update your authentication flow

-- === JWT ROLE SETUP ===
-- If you need to add role to JWT claims, you can create a trigger function:
/*
...
-- Then set it in auth config (Dashboard > Authentication > Hooks)
*/